...
//Jo-Pippin 
//@version=5
indicator(title='Mondays | Opens V3', shorttitle='Mondays | Opens V3', overlay=true , max_bars_back = 500)
...
inp_line_style_poc     = input.string("Dashed", options = ["Dotted", "Solid", "Dashed"], title = "Poc Line Style", group = "Monday Settings")
...
show_vol_candle_second = input.bool(true, 'Show Max Vol candle measured from the start of this week', group = "Volume & OI Settings")
...
wick_width             = input.int (2,    'Volume Wick width on existing candles',     minval=1, maxval=30, group = "Volume & OI Settings")
body_width             = input.int (5,    'Volume Body width on existing candles',     minval=1, maxval=30, group = "Volume & OI Settings")

break_line_col         = input.color(color.new(#afcff7, 84), 'Day Break Color', group = "Color Settings")
day_label_col          = input.color(color.new(#afcff7, 10), 'Day Label Color', group = "Color Settings")
Mon_lines              = input.color(color.new(#afcff7, 84), 'Monday Line Color', group = "Color Settings")
Opening_R              = input.color(color.new(#434651, 70), 'Monday Opening Range color', group = "Color Settings")
...
// Rise_c                 = input.color(color.new(#66bb6a, 0),  'OI Rise Candle Color', group = "Color Settings")
// Fall_c                 = input.color(color.new(#f7525f, 0),  'OI FallCandle Color', group = "Color Settings")
...
mainpoc_c              = input.color(color.new(#bec2cc, 15) ,'Main Poc Color', group = "Color")
...
tostring_alert = first_Highest_index  == 0   ? "Weekly volume on the M" + str.tostring(timeframe.period) + " candle just shifted to the current candle" : na
